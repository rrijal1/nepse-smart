...
class DataManager:
    ...
    def migrate_json_to_postgres(self) -> Dict[str, Any]:
        ...
        # Find all JSON files with historical data - handle both naming patterns
        json_files = []
        json_files.extend(daily_dir.glob("*_price_volume_history.json"))  # Pattern: YYYY_MM_DD_price_volume_history.json
        json_files.extend(daily_dir.glob("*_price_volume_history_*.json"))  # Pattern: YYYY-MM-DD_price_volume_history_YYYY_MM_DD.json
        ...
        for json_file in json_files:
            try:
                ...
                if 'content' in file_data and isinstance(file_data['content'], list):
                    data = file_data['content']
                elif isinstance(file_data, list):
                    data = file_data
                else:
                    errors.append(f"Unexpected JSON structure in {json_file.name}")
                    continue
